...
class FaceRec:
    ...
    def doCalculations(self, dir, imglist, selectednumeigenfaces):
        ...
        self.weights = self.calculateWeights(
            egfaces, adjfaces, selectednumeigenfaces)
    ...
    def validateDirectory(self, imgfilenameslist):
        ...
        for x in imgfilelist:
            ...
            if((newwdth != imgwdth) or (newht != imght)):
                raise DirError(
                    "select folder with all images of equal dimensions !")
        return imgfilelist
    ...
    def createFaceBundle(self, imglist):
        ...
        # trying to create a 2d array ,each row holds pixvalues of a single
        # image
        ...
        # substract avg val from each orig val to get adjusted faces(phi of
        # T&P)
        ...
        self.bundle = FaceBundle(
            imglist, imgwdth, imght, adjfaces, u, avgvals, evals)
    ...
    def reconstructfaces(self, selectedfacesnum):
        ...
        for x in range(numimgs):
            ...
            imgdata = phinew[x]
            imageops.make_image(
                imgdata, imgname, (self.bundle.wd, self.bundle.ht), True)

        for x in range(numimgs):
            filename = recondir + "/reconx" + str(x) + ".png"
            imgdata = xnew[x]
            imageops.make_image(
                imgdata, filename, (self.bundle.wd, self.bundle.ht), True)
    ...
    def createEigenimages(self, eigenspace):
        ...
        for x in range(numimgs):
            ...
            imageops.make_image(
                eigenspace[x], imgname, (self.bundle.wd, self.bundle.ht))
    ...
    def checkCache(self, dir, ext, imglist, selectedfacesnum, thrval):
        ...
        try:
            ...
            if(imglist == oldlist):
                ...
                adjfaces = self.bundle.adjfaces
                self.weights = self.calculateWeights(
                    eigenfaces, adjfaces, selectedfacesnum)
            if(cache_changed):
